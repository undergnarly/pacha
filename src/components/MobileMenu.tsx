...
export default function MobileMenu({ open, onClose }: MobileMenuProps) {
  ...
  const links = [
    ...
    { label: t.ui.alpacafe, href: "/alpacafe" },
    { label: t.ui.lodges, href: "/lodges" },
    { label: t.ui.privateDinner, href: "/dinner" },
  ];
          ...
          <div className="flex flex-col items-center gap-6 pb-10">
            ...
            >
              {(Object.keys(localeLabels) as Locale[]).map((l) => (
                <button
                  key={l}
                  onClick={() => setLocale(l)}
                  className={`rounded-full px-4 py-1.5 text-xs font-medium uppercase tracking-widest transition-all ${
                    locale === l
                      ? "bg-white text-black"
            ...
            <div className="flex justify-center gap-8">
              ...
              <a
                ...
                className="text-white/50 transition-colors hover:text-white"
              >
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="M22 4L12 13 2 4"/></svg>
              </a>
            </div>
          </div>
        </motion.div>
